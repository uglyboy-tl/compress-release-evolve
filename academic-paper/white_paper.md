...
**The universal paradigm of evolution.** When coding evolution is itself analyzed as a system, the theory's diagnostic lens reveals a structural constraint that yields the universal evolutionary paradigm. The "dimension" being operated on at this meta-level is the code-space itself — the set of all possible control rules. A compression mapping on the code-space takes the form of a new code: a constraint that reduces the entropy of expression in some sub-dimension. Forming new codes is not merely *one* way to control the code-space; it is the *only* viable one, because any other control operation at this level would itself be describable as a code — a constraint on which codes are permitted — and would therefore still be a compression mapping on the code-space. The entropy displaced by this compression does not vanish. It is released into the code-combination dimension, generating an explosion of new code-code interactions. This is the structural reason the positive-feedback cycle (new code → new combinations → new codes) takes the form it does. It is not one possible configuration among many. It is the necessary consequence of applying the theory's own compression-mapping logic to the code-space itself. The three-beat cycle — entropy decrease through new codes, entropy increase through new combinations, new entropy decrease through yet newer codes — is therefore the universal paradigm of evolution, derivable from the theory's own axioms rather than observed as an empirical regularity.
